 ...
 *
 * Reps × weight, weighted rep work only — the same rule the live session's
 * `totalVolumeKg` uses, so the chart and the history row agree. A bodyweight-only
 * day has no volume to plot and is dropped for the reason above.
 */
export function workoutVolumeSeries(workouts: readonly CompletedWorkout[]): TrendPoint[] {
  ...
  for (const workout of workouts) {
    let volume = 0;
    for (const row of workout.sets) {
      if (row.countUnit !== 'reps' || row.weightKg == null || !counts(row)) continue;
      volume += row.weightKg * row.count;
    }
    if (volume > 0) points.push({ at: workout.startedAt, value: Math.round(volume) });
  }
  return points.reverse();
}
 ...
 */
export function exerciseCountSeries(
