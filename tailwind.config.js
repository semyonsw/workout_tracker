...
module.exports = {
  ...
  theme: {
    ...
    //
    // The GLASS redesign deliberately did not add a fifth.
    // Its softer corners live in `radius` in `theme/tokens.ts` as NUMBERS,
    // because a `BlurView` has to be told its corner in `style` — NativeWind's
    // transform does not reach a native component — and a second spelling of
    // the same scale is a second thing to keep in sync for nobody's benefit.
    borderRadius: {
      none: '0',
    ...
    fontSize: {
      ...
      // The glass redesign's four FOCAL steps are not here either, and for the
      // same reason as its radii: `focalType` in `theme/tokens.ts` holds them as
      // numbers because a screen has to reason about them — the day dial sizes
      // itself against the ring it sits in, the keypad's amount against its own
      // width — and a className cannot be measured.
    },
    spacing: {
      0: '0px',
